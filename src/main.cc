...
namespace
{
  ...
  void print_help(VS_FIXEDFILEINFO *file_info)
  {
    fprintf(stdout,
            ...
            "  --set-language-and-codepage <languageId> <codePage>  Set language ID and code page\n"
...
int wmain(int argc, const wchar_t *argv[])
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    else if (wcscmp(argv[i], L"--set-language-and-codepage") == 0 ||
             wcscmp(argv[i], L"-slc") == 0)
    {
      if (argc - i < 3)
        return print_error("--set-language-and-codepage requires 'languageId' and 'codePage'");

      const wchar_t *langIdStr = argv[++i];
      const wchar_t *codePageStr = argv[++i];

      unsigned int langId = 0;
      unsigned int codePage = 0;

      if (swscanf_s(langIdStr, L"%d", &langId) != 1)
        return print_error("Unable to parse languageId");

      if (swscanf_s(codePageStr, L"%d", &codePage) != 1)
        return print_error("Unable to parse codePage");

      if (!updater.SetLanguageAndCodePage(static_cast<LANGID>(langId), static_cast<WORD>(codePage)))
        return print_error("Unable to set language ID and code page");
    }
    else
    {
      if (loaded)
