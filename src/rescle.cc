...
#pragma pack(pop)
  ...
  bool ResourceUpdater::SetLanguageAndCodePage(LANGID languageId, WORD codePage)
  {
    // If the version info for this language doesn't exist, create one using default values
    if (versionStampMap_.find(languageId) == versionStampMap_.end())
    {
      versionStampMap_[languageId] = VersionInfo();
    }

    // If string tables for this language don't exist, we'll create them when needed

    // Update existing translations or add a new one
    bool translationExists = false;
    for (auto &translate : versionStampMap_[languageId].supportedTranslations)
    {
      if (translate.wLanguage == languageId)
      {
        // Update existing translation
        translate.wCodePage = codePage;
        translationExists = true;
        break;
      }
    }

    // If no translation found for this language, add it
    if (!translationExists)
    {
      Translate newTranslate = {languageId, codePage};
      versionStampMap_[languageId].supportedTranslations.push_back(newTranslate);
    }

    // Update encoding for all string tables for this language
    for (auto &stringTable : versionStampMap_[languageId].stringTables)
    {
      stringTable.encoding.wLanguage = languageId;
      stringTable.encoding.wCodePage = codePage;
    }
